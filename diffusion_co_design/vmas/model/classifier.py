...
class SimpleSpreadEnvCritic(_EnvCritic):
    def __init__(
        self,
        ...
        n_goals = scenario.get_num_agents()
        assert scenario.diffusion_shape[0] == n_goals * 2
        # Positions + Goals + Pairwise distances between goals
        dim_in = int(n_goals * 2 + (n_goals * (n_goals - 1) / 2))
    ...
    def forward(self, x: torch.Tensor, t=None):
        # Share agent observations
        B_all = x.shape[:-1]
        n_goals = self.scenario.get_num_agents()
        assert x.shape == (*B_all, self.scenario.diffusion_shape[0])
        assert x.shape == (*B_all, n_goals * 2)

        # Calculate pairwise distances between goals
        goal_pos = x.view(*B_all, n_goals, 2)
        goal_dists = torch.cdist(goal_pos, goal_pos, p=2)

        goal_dists_flat = goal_dists.flatten(start_dim=-2, end_dim=-1)
        # Remove self-distances
        mask = (
            torch.ones((n_goals, n_goals), device=x.device)
            .tril(diagonal=-1)
            .bool()
            .flatten()
        )
        goal_dists_flat = goal_dists_flat[..., mask]

        assert goal_dists_flat.shape[-1] == (n_goals * (n_goals - 1)) // 2
        x_in = torch.cat([x, goal_dists_flat], dim=-1)
