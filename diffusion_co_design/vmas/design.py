...
class ValueLearner(design.ValueLearner):
    def __init__(
        ...
        elif isinstance(scenario, LocalPlacementScenarioConfig) or isinstance(
...
class SamplingDesigner(design.SamplingDesigner[SC]):
    ...
    def generate_random_layouts(self, batch_size):
        return torch.tensor(
            create_generate(self.scenario)(n=batch_size, training_dataset=False),
            dtype=torch.float32,
        )
...
class DescentDesigner(design.GradientDescentDesigner[SC]):
    def __init__(
        ...
        if isinstance(self.scenario, GlobalPlacementScenarioConfig):
            self.pc = soft_projection_constraint(cfg=self.scenario)
        else:
            # For local/simple_spread the train-space layout lives in [-1, 1].
            # Clamp after each gradient epoch to prevent the optimiser from
            # escaping the training distribution and exploiting extrapolation.
            self.pc = lambda x: torch.clamp(x, -1.0, 1.0)

    def _generate_initial_env(self, batch_size):
        layouts = create_generate(self.scenario)(n=batch_size, training_dataset=False)
        env = torch.tensor(layouts, dtype=torch.float32, device=self.value_learner.device)
        return eval_to_train(env=env, cfg=self.scenario)

    def _train_to_eval(self, env):
        return train_to_eval(env=env, cfg=self.scenario)
...
def create_designer(
    ...
    if isinstance(designer, schema.Fixed):
        ...
        elif isinstance(designer, SamplingConfig):
        ...
        elif isinstance(designer, DescentConfig):
