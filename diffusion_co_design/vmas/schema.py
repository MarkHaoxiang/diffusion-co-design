...
# TODO markli: this is so hacky. We should move to a proper schema registry, attached to each environment.
class SimpleSpreadScenarioConfig(BaseScenarioConfig):
    name: str
    episode_steps: int
    n_agents: int
    placement_area: Literal["simple_spread"] = "simple_spread"
    ...
    @property
    def layout_space_high(self) -> torch.Tensor:
        return torch.ones((self.diffusion_shape[0],))
