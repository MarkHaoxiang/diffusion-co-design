...
class DicodeDesigner(design.DicodeDesigner[SC]):
    def __init__(
        ...
        # Backward guidance (disable_pug=True) requires a time-conditioned value
        # model: switch to UnetCNNClassifier which has sinusoidal timestep embeddings,
        # and enable forward-process noise augmentation during value training.
        vl_hp = value_learner_hyperparameters
        if diffusion.disable_pug:
            classifier = classifier.model_copy(update={"name": "unet-cnn"})
