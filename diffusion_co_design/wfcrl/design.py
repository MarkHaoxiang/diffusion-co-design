    ...
    def __init__(
        ...
        super().__init__(
            model=GNNCritic(
                ...
                include_timestep_embedding=include_timestep_embedding,
            ),
...
class DescentDesigner(design.GradientDescentDesigner[SC]):
    ...
    def generate_random_layouts(self, batch_size, training):
        layouts = torch.tensor(
            self.generate(
                n=batch_size,
    ...
    def _generate_initial_env(self, batch_size):
        return self.generate_random_layouts(batch_size, training=True)

    def _generate_random_layout_batch(self, batch_size: int):
        return list(self.generate_random_layouts(batch_size), training=False)

    def _train_to_eval(self, env):
        return train_to_eval(env, self.scenario)
...
class DicodeDesigner(design.DicodeDesigner[SC]):
    def __init__(
        ...
        # When using backward guidance (disable_pug=True) the value model
        # receives noisy x_t during generation, so it must be trained with
        # the same noise distribution via q_sample.
        vl_hp = value_learner_hyperparameters
        if diffusion.disable_pug:
            vl_hp = dataclasses.replace(vl_hp, noise_augmentation=True)
        ...
        super().__init__(
            ...
            value_learner=ValueLearner(
                ...
                classifier=classifier,
                normalisation_statistics=normalisation_statistics,
                hyperparameters=vl_hp,
                include_timestep_embedding=diffusion.disable_pug,
                gamma=gamma,
                device=device,
            ),
        ...
        # Wire the generator's diffusion into the value learner for noise augmentation.
        if diffusion.disable_pug:
            self.value_learner.initialise_noise_augmentation(self.generator.diffusion)
