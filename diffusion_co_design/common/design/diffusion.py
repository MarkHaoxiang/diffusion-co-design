...
class BaseGenerator(ABC):
    ...
    def generate_batch(
        ...
        if value is not None:
            # SpacedDiffusion wraps cond_fn with _WrappedModel which remaps
            # spaced indices to original-scale indices before calling cond_fn,
            # so normalize by original_num_steps - 1 to get t_normalized in [0, 1].
            _t_norm_scale = (
                self.diffusion.original_num_steps - 1
                if isinstance(self.diffusion, SpacedDiffusion)
                else self.diffusion.num_timesteps - 1
            )

            def cond_fn(x: torch.Tensor, t):
                t_normalized = t.float() / _t_norm_scale

                with torch.enable_grad():
                    x_in = x.detach().requires_grad_(True)
                    out = (
                        value.predict_theta_value(x_in, t_normalized).sum()
