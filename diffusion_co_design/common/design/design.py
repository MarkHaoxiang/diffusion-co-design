...
class ValueLearner[SC: ScenarioConfig]:
    def __init__(
        ...
        # Forward-process noise augmentation for time-conditioned value learning.
        # Call initialise_noise_augmentation(diffusion) before the first update.
        self.noise_augmentation = hp.noise_augmentation
        self._noise_augmentation_initialised = False

    def initialise_noise_augmentation(self, diffusion):
        assert self.noise_augmentation, (
            "noise_augmentation must be True in hyperparameters"
        )
        assert not diffusion.rescale_timesteps, (
            "rescale_timesteps=True misaligns training t with cond_fn t"
        )
        self._diffusion = diffusion
        self._schedule_sampler = create_named_schedule_sampler("uniform", diffusion)

        # SpacedDiffusion wraps cond_fn with _WrappedModel which remaps spaced
        # indices [0, num_timesteps) → original indices [0, original_num_steps).
        # Store the map and scale so training and inference normalize t identically.
        if hasattr(diffusion, "timestep_map"):
            self._t_norm_scale = diffusion.original_num_steps - 1
            self._t_map = torch.tensor(diffusion.timestep_map, dtype=torch.long)
        else:
            self._t_norm_scale = diffusion.num_timesteps - 1
            self._t_map = None

        self._noise_augmentation_initialised = True
    ...
    def update(self, td: TensorDict):
        ...
        if len(self.env_buffer) >= max((self.train_batch_size, self.train_early_start)):
            ...
            for _ in range(self.n_update_iterations):
                ...
                # Apply forward-process noise augmentation when enabled.
                # q(x_t | x_0) = sqrt(alpha_bar_t)*x_0 + sqrt(1-alpha_bar_t)*eps
                # The model learns V(x_t, t) -> V(x_0) across the full noise schedule.
                assert (
                    not self.noise_augmentation or self._noise_augmentation_initialised
                ), "Call initialise_noise_augmentation(diffusion) before update"
                if self.noise_augmentation:
                    t_aug, _ = self._schedule_sampler.sample(
                        X_batch.shape[0], X_batch.device
                    )
                    X_batch = self._diffusion.q_sample(
                        X_batch, t_aug, noise=torch.randn_like(X_batch)
                    )
                    # Map spaced indices to original scale so t_normalized matches
                    # what cond_fn receives (SpacedDiffusion._wrap_model remaps
                    # spaced → original before calling cond_fn).
                    if self._t_map is not None:
                        t_orig = self._t_map.to(X_batch.device)[t_aug]
                    else:
                        t_orig = t_aug
                    t_normalized = t_orig.float() / self._t_norm_scale
                else:
                    t_normalized = None

                y_pred, hint_pred = self.model.predict_theta_value_with_hint(
                    X_batch, t_normalized
                )
